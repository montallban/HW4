...
def read_images_from_directory(directory, file_regexp):
    ...
    list_of_images = [readPngFile(directory + slash + f) for f in files if re.search(file_regexp, f)]
...
def read_image_set_from_directories(directory, spec):
    ...
    '''
    out = read_images_from_directory(directory + slash + spec[0][0], spec[0][1])
    for sp in spec[1:]:
        out = np.append(out, read_images_from_directory(directory + slash + sp[0], sp[1]), axis=0)
    return out
...
def load_multiple_image_sets_from_directories(directory_base, directory_list, object_list, test_files):
    ...
    # First directory
    ret = read_image_set_from_directories(directory_base + slash + directory_list[0], inputs)
    
    # Loop over directories
    for directory in directory_list[1:]:
        ret = np.append(ret,
                        read_image_set_from_directories(directory_base + slash + directory, inputs),
                        axis=0)
