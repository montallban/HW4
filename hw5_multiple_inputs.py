...
def load_data_sets(directory_base, args, objects=None, condition_list=None, file_spec=None):
    ...
    ins2_train = np.concatenate(ins2_train, axis=0)
    outs_train = np.concatenate(outs_train, axis=0)

    ins1_val = np.concatenate(ins1_val, axis=0)
    ins2_val = np.concatenate(ins2_val, axis=0)
    outs_val = np.concatenate(outs_val, axis=0)
    
    return ins1_train, ins2_train, outs_train, ins1_val, ins2_val, outs_val

def create_parser():
    # Parse the command-line arguments
    parser = argparse.ArgumentParser(description='BMI Learner', fromfile_prefix_chars='@')
    parser.add_argument('-network',type=str,default='multi_image_inception',help="Choose shallow, deep, or inception")
    ...
    parser.add_argument('-results_path', type=str, default='./results_hw5', help='Results directory')
...
def generate_fname(args, params_str):
    ...
    filters_str = '_'.join(str(x) for x in args.filters)
...
def execute_exp(args=None):
    ...
    # Load data
    ins1, ins2, outs, ins1_validation, ins2_validation, outs_validation = load_data_sets(args.dataset, args)
    image_size=(ins1.shape[1], ins1.shape[2])
    nchannels = ins1.shape[3]

    print('TR:', ins1.shape)
    print('V:', ins1_validation.shape)
    ...
    if args.network == "shallow":
        ...
        model = create_inception_network(image_size, nchannels, args.filters,
                                          lambda_regularization=args.L2_regularizer)

    elif args.network == "multi_image_inception":
        model = create_dual_input_network(image_size, nchannels, args.filters,
                                            lambda_regularization=args.L2_regularizer)
    ...
    generator = training_set_generator_images(ins1, ins2, outs, batch_size=args.batch,
                            input_1_name='input_1',
                            input_2_name='input_2',
                            output_name='output')
    ...
    history = model.fit(x=generator, 
                        ...
                        validation_data=([ins1_validation, ins2_validation], outs_validation), 
    ...
    results = {}
    results['args'] = args
    results['predict_training'] = model.predict([ins1, ins2])
    results['predict_training_eval'] = model.evaluate([ins1, ins2], outs)
    results['true_training'] = outs
    results['predict_validation'] = model.predict([ins1_validation, ins2_validation])
    results['predict_validation_eval'] = model.evaluate([ins1_validation, ins2_validation], outs_validation)
